...
regen_rootfs()
{
	...
	if [ "$PLATFORM" != "sun4i_crane" -a   "$PLATFORM" != "a13_nuclear" -a  "$PLATFORM" != "a12_nuclear"  -a  "$PLATFORM" != "a13-test"  -a  "$PLATFORM" != "sun6i" ]; then
		echo "Regenerating Rootfs..."
        if [ "$PLATFORM" == "a13_dragonboard" ]; then
